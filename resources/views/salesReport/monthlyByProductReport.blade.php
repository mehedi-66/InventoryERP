...
@section('main-section')
    ...
    <div class="px-5">
        <div class="row mb-2">
            ...
            <div class="col-11">
                <h6 class="text-center " style="letter-spacing: 1px;">Product Sales on {{ \Carbon\Carbon::parse($FormDate)->startOfMonth()->format('jS') }} to {{ \Carbon\Carbon::parse($FormDate)->endOfMonth()->format('jS F Y') }}</h6> 
            </div>
            <div class="col-1">
                ...
                <select id="product_id" name="product_id" class="form-control">
                    ...
                    @foreach($productList as $product)
                        <option value="{{ $product->product_id }}"
                            {{ old('product_id', $product_id ?? '') == $product->product_id ? 'selected' : '' }}>
         ...
         <table id="myTable" class="table-bordered">
            ...
            <tbody>
                @foreach ($salesInvoiceList as $prod)
                    ...
                    <td>{{$prod->product_unit_price * $prod->product_quantity}}</td>
                    </tr>
                @endforeach
            </tbody>
        </table> 
    ...
    <script type="text/javascript">
        ...
        function Search() {
            ...
            console.log(date);
            window.location.href = "{{ url('/report/byProducthMontlySales') }}/" + date + "/"+product_id;
        }
    </script>
